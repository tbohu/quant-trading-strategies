#!/bin/bash
# 旺财AI量化分析系统 V8.0 一键部署
set -e
echo "======================================="
echo "  旺财AI量化系统 V8.0 一键部署"
echo "======================================="
...
cp server_v8.py server.py
cp index_v8.html static/index.html

cat > /var/www/wangcai/.env << 'ENV'
WANGCAI_TOKEN=Wangcai_1dcRzE-OSUOcRhrp99ZjxiWeyzxR2cHwnyQPWNhAnpQ
...
DB_PATH=/var/www/wangcai/wangcai_v8.db
WECOM_KEY=
ENV

cat > /etc/systemd/system/wangcai.service << 'SVC'
[Unit]
Description=旺财AI量化分析系统V8
...
if curl -s -H "X-Wangcai-Token: Wangcai_1dcRzE-OSUOcRhrp99ZjxiWeyzxR2cHwnyQPWNhAnpQ" http://localhost:7777/health | grep -q "ok"; then
    echo "======================================="
    echo "  ✅ V8.0 部署成功！"
    ...
    echo "❌ 启动失败：journalctl -u wangcai -n 50 --no-pager"
fi
